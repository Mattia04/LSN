...
void System ::initialize_velocities()
{
	...
	else
	{
		...
		double velocity = sqrt(3 * _temp);
		int direction;

		// TODO TO BE TEMPORARILY CHANGED IN EXERCISE 4
		for (int i = 0; i < _npart; i++)
		{
			direction = _rnd.Rannyu(0, 6);
	...
	if (input.is_open())
	{
		while (!input.eof())
		{
			...
			else if (property == "POFV")
			{
				...
				input >> _n_bins_v;
				_nprop += _n_bins_v;
				_bin_size_v = 4.0 * sqrt(_temp) / (double)_n_bins_v; // TODO TO BE FIXED IN EXERCISE 4
				_measure_pofv = true;
				_index_pofv = index_property;
				index_property += _n_bins_v;

				coutpv << setw(12) << "BINSIZE: " << _bin_size_v << endl;
...
void System ::read_configuration()
{
	...
	if (cinf.is_open())
	{
		...
		for (int i = 0; i < _npart; i++)
		{
			...
			_particle(i).setposition(0, this->pbc(0.5 * _side(0) * x, 0)); // TODO remove 0.5
...
{ // Measure properties
	...
	double virial = 0.0;
	unsigned int pofv_temp = 0; // temporary accumulator for pofv
	...
	if (_measure_pofv)
	{
		for (int i = 0; i < _npart; i++)
		{
			pofv_temp = sqrt(dot(_particle(i).getvelocity(), _particle(i).getvelocity())) / _bin_size_v;
			if (pofv_temp >= _n_bins_v)
				continue;

			_measurement(_index_pofv + pofv_temp)++;
		}
	}
	// POTENTIAL ENERGY //////////////////////////////////////////////////////////
	if (_measure_penergy)
...
void System ::averages(int blk)
{
	...
	if (_measure_pofv)
	{
		...
		for (int i = 0; i < _n_bins_v; i++)
		{
			...
			coutf << setw(12) << blk
				  << setw(12) << average / (_npart * _bin_size_v)
				  << setw(12) << sum_average / double(blk) / (_npart * _bin_size_v)
				  << setw(12) << this->error(sum_average, sum_ave2, blk) / (_npart * _bin_size_v) << endl;
		}
		coutf.close();
	}
	// MAGNETIZATION /////////////////////////////////////////////////////////////
