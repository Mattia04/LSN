...
class City
{
    static unsigned int s_next_uid; // for uid generator
    const Position m_pos;           // position of the city
    const unsigned int UID;         // uid of the city
...
class Cities
{
    const std::vector<City> m_cities;                              // vector of cities
    const std::vector<unsigned int> m_possible_UIDs;               // vector of cities uids
...
public:
    ...
    /**
     * @brief Construct a collection of cities from the positions contained inside a file
     * @param filename the name of the file where to load the positions.
     * @return a Cities instance.
     */
    static Cities Initialize_From_File(const char *filename);
    ...
    /// The file should contain two columns of float with x and y coordinates of the cities, nothing else.
    /// @param filename the name of the file where to save the positions.
    /// @note this method assumes that file exists and the cities are in the correct format
