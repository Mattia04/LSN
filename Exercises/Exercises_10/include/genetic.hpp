...
public:
    ...
    ///     \f[j = \mathrm{population size} \cdot \mathrm{floor}(r ^ p)\f]
    ...
    /// @brief computes the fitness of the first chromosome in the population.
    /// Also inserts the fitness value in the correct position in the sorted fitness vector.
    /// @param map the map containing the city coordinates and distances
    void calculate_fitness_first(const Cities &map);
    ...
    /// @brief overwrites the first chromosome with the new route
    /// @param other_string a vector of the new route.
    void overwrite_first_chromosome(const std::vector<unsigned int> &other_string);
};
