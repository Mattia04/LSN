...
public:
    ...
    /// @brief sets the chromosome to a new given route
    /// @param new_string a vector representing the route of cities to visit
    void overwrite_string(std::vector<unsigned int> new_string) { m_string = new_string; };

    /// @brief gets the route of the chromosome
    /// @return a vector representing the route of cities to visit
    [[nodiscard]] std::vector<unsigned int> &get_string() { return m_string; }
};
