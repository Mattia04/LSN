...
void Genetic::calculate_fitness_first(const Cities &map)
{
    m_fitness[0] = m_chromosomes[0].L1(map);

    // Insert the fitness value in the correct position in the sorted fitness vector
    std::inplace_merge(m_fitness.begin(), m_fitness.begin() + 1, m_fitness.end());
}
...
void Genetic::overwrite_first_chromosome(const std::vector<unsigned int> &best_chromosome)
{
    m_chromosomes[0].overwrite_string(best_chromosome);
}
