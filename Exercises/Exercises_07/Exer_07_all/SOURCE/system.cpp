	...
	if (input.is_open())
	{
		while (!input.eof())
		{
			...
			if (property == "POTENTIAL_ENERGY")
			{
				...
				index_property++;
				_vtail = 8 * M_PI * _rho * (pow(_r_cut, -9) / 9. - pow(_r_cut, -3) / 3.); // TO BE FIXED IN EXERCISE 7
			}
...
{ // Measure properties
		...
		for (int i = 0; i < _npart - 1; i++)
		{
			for (int j = i + 1; j < _npart; j++)
			{
				...
				if (dr < _r_cut)
				{
					...
					if (_measure_gofr)
						_measurement(_index_gofr + int(dr / _bin_size)) += 2.;
				}
			}
		}
	}
	...
	// SUSCEPTIBILITY ////////////////////////////////////////////////////////////
	if (_measure_chi)
		_measurement(_index_chi) = _beta * _measurement(_index_magnet) * _measurement(_index_magnet) * _npart;
...
void System ::averages(int blk)
{
	...
	if (_measure_gofr and blk == _nblocks)
	{
		...
		for (int i = 0; i < _n_bins; i++)
		{
			...
			double norm = (_rho * _npart * 4. * M_PI / 3. * (pow(r + _bin_size, 3) - pow(r, 3))); // rho * N * delta V
	...
	if (_measure_cv)
	{
		// from _global_av i need to remove <H^2> manually from the average of C
		_global_av(_index_cv) -= _average(_index_cv);
		_global_av2(_index_cv) -= _average(_index_cv) * _average(_index_cv);
		// calculate our variable of interest using <H^2> - <H>^2
		average = pow(_beta, 2) * (_average(_index_cv) - _average(_index_tenergy) * _average(_index_tenergy)) * _npart;
		// add it to the datablocking
